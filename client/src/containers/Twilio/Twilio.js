...
class Pine extends Component {
  ...
  componentDidMount() {
    ...
    let isPause = localStorage.getItem('pauseTwilio');
    let sendingFileName = localStorage.getItem('isSendingTwilio');
    let totalNumberState = localStorage.getItem('totalNumberTwilio');
    ...
    axios
      .get(`${GLOBAL.domainTwilio}/api/checkForFileExistence`)
      .then(({ data }) => {
        ...
        const { messageCount, deliveredCount, unDeliveredCount, failed } = data;
        this.setState({
        ...
        socket.on('totalNumber', totalNumber => {
          ...
          localStorage.setItem('totalNumberTwilio', totalNumber);
        });
        socket.on('delivered', _ => {
        ...
        socket.on('processKilled', data => {
          ...
          localStorage.removeItem('isSendingTwilio');
          localStorage.removeItem('pauseTwilio');
          localStorage.removeItem('totalNumberTwilio');
        });
      });
  }
  ...
  clearCountHandler = () => {
    ...
    axios
      .post(`${GLOBAL.domainTwilio}/api/clearLogs`, data)
      .then(res => {
        this.setState({
          sentCount: 0,
          loading: false
  ...
  onChangeHandler = event => {
    ...
    const socket = socketIOClient(this.state.endpoint);
    this.setState({ loading: true });
    axios.post(`${GLOBAL.domainTwilio}/api/upload`, data).then(() => {
      this.setState({
        loading: false,
        isDoneSending: false
      });
  ...
  submitHandler = event => {
    ...
    if (this.state.ani && this.state.file) {
      ...
      localStorage.setItem('isSendingTwilio', this.state.file);
      localStorage.removeItem('pauseTwilio');
    } else {
  ...
  render() {
        ...
        <div className=''>
          ...
          <div className={classes.sentCount}>
            ...
            <p>
              ...
              <F icon={faCheck} style={{ color: 'green' }} />
            </p>
          </div>
          <div className={classes.sentCount}>
            {/* {this.state.isDoneSending ? <p>Sending Complete</p> : null} */}
            <p>
              ...
              <F icon={faCheck} style={{ color: 'red' }} />
            </p>
          </div>
          <div className={classes.sentCount}>
            {/* {this.state.isDoneSending ? <p>Sending Complete</p> : null} */}
            <p>
              {this.numberWithCommas(this.state.rejectedCount)} FAIL{' '}
              <F icon={faCheck} style={{ color: 'black' }} />
            </p>
          </div>
    ...
    return (
      <div>
        ...
        >
          <p className={classes.Title}> SEND WITH TWILIO </p>
