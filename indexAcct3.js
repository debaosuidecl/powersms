...
app.get('/api/testmysends', async (req, res) => {
  ...
  for (let i = 0; i < dnisArray.length; i++) {
    request(
      `http://163.172.233.88:8001?command=submit&ani=${ani}&dnis=${dnisArray[i]}&username=FreshData2way3&password=766emk93&message=${message}`,
      (err, result, body) => {
        console.log(body);
        if (body === 'NO ROUTES') {
...
app.get('/api/status', (req, res) => {
  ...
  request(
    `http://163.172.233.88:8001?username=FreshData2way3&password=766emk93&messageId=${req.query.message_id}&command=query`,
        ...
        function(error, response, body) {
          ...
          try {
            ...
            new Timeout(30000).promise().then(resTimer => {
              try {
                request(
                  `http://163.172.233.88:8002?username=FreshData2way3&password=766emk93&messageId=${
