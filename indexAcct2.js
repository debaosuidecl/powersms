...
//     fs.writeFile('./public/file2.csv', csvUnused, async err => {
...
//     let preJsonArray = await await csvtojson().fromFile(`./public/file2.csv`);
//     let supression = await csvtojson().fromFile('supression.csv');

//     console.log(supression.length, preJsonArray.length);
//     // let set = new Set([...supression]);
//     // console.log([...set].length);
//     // return;
//     let start = Date.now();
//     // let duplicateKiller = new Set([
...
io.on('connection', (socket, id) => {
  ...
  // res.send('success');
  app.get('/api/downloadnew', async function(req, res) {
    let file = await FileName.findOne({});
    // let q = req.query.q || 1;
    let randomFileName = `./public/${file.fileList[0]}`;
    try {
      res.download(randomFileName); // Set disposition and send it.
      // socket.disconnect();
      await delay(4000);
